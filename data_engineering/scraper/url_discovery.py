...
# 구글 검색으로도 안 나오는 기관들을 위한 최후의 보루 (하드코딩)
HARDCODED_URLS = {
    "강원특별자치도": ["https://job.gwd.go.kr/youth"], # 검색 안될 때 강제 주입
}
...
def cleanup_url(url):
    """
    URL 끝에 붙는 불필요한 경로(error.html, index.html 등)를 제거합니다.
    """
    if not url: return None
    
    trash_patterns = [
        r'/error\.html$', r'/error\.jsp$'
    ]
...
def extract_top_results(organic_results, top_n=2):
    """
    스코어링 없이 구글이 준 순서대로 상위 N개만 뽑습니다.
    """
    if not organic_results: return []
    # 구글의 순서를 그대로 믿고 상위 N개의 link만 추출
    return [item.get('link') for item in organic_results[:top_n]]
...
def call_lambda_clone(query):
    try:
        ...
        if result.get('statusCode') == 200:
            ...
            organic_data = body.get('organic', [])
            # 구글의 상위 2개 결과를 그대로 가져옵니다.
            return extract_top_results(organic_data, top_n=2)
        return []
...
def process_single_target(target):
    ...
    if name in HARDCODED_URLS:
        return {"nm": name, "urls": HARDCODED_URLS[name]}
    
    is_hub = target.get('type') == 'hub'
    query = f"{name} {YOUTH_KEYWORDS if not is_hub else ''}".strip()
    
    raw_urls = call_lambda_clone(query)
    
    validated_urls = []
    for url in raw_urls:
        # 1. 리다이렉트 추적
        final_url = validate_and_follow_redirect(url)
        # 2. 불필요한 꼬리표(error.html 등) 제거
        cleaned_url = cleanup_url(final_url)
        
        if cleaned_url and cleaned_url not in validated_urls:
            validated_urls.append(cleaned_url)
            
    return {"nm": name, "urls": validated_urls}
